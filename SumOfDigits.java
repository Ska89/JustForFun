 ...
 * by playing with decimal system which we use everyday (though you should keep in mind that computer does not use
 * it internally - it only converts numbers to it when they should be shown to user).
 ...
 Important: while many programming languages have built-in functions to convert numbers to strings (from which digits
 could be extracted), you should not use this (since your goal is to learn some programming tricks).

 Instead you need to implement algorithm with repetitive division by 10 (base of numeral system) and summing up the
 remainders. Read the Number to digits article for details on the algorithm.
...
public class SumOfDigits {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            sb.append(calculateSumOfDigits(sc.nextInt(), sc.nextInt(), sc.nextInt()));
            if (i < count - 1) sb.append(" ");
        }
        printResult(sb);
    }

    private static int calculateSumOfDigits(int a, int b, int c) {
        int d = a * b + c, res = 0;
        while (d != 0) {
            res += d % 10;
            d /= 10;
