 ...
 * - and because the proof is still unknown. However it offers good programming exercise for beginners.
 ...
 Answer should contain the same amount of results, each of them being the count of steps for getting Collatz Sequence
 to 1.
...
public class CollatzSequence {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            sb.append(countOfStepsToOne(sc.nextInt()));
    ...
    private static int countOfStepsToOne(int x) {
        int count = 0;
        while (x != 1) {
            if (x % 2 == 0) x /= 2;
            else x = x * 3 + 1;
