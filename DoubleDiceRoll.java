 ...
 *
 * After solving Dice Rolling, we already know how to convert fractional value between 0.0 and 1.0 to an integer number
 * from within the specific range - for example to values between 1 and 6, simulating the throwing of the die.

 However, many programming languages (like C/C++, PHP, Pascal) use random number generator which on contrary gives only
 integer values in the range from 0 to some (very large) maximum. How to convert such numbers to dice points?

 It happens that this case is even simpler. We can use the following approach:

 Divide the random value R by the N (number of distinct values we want - e.g. 6 for dice) and take the remainder. This
 remainder is necessarily the integer value in the range from 0 to N - 1.
 Now to shift it to the necessary range, simply add 1 (i.e. the minimum of the range we want) - and you'll get the value
...
public class DoubleDiceRoll {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            sb.append(calculateDoubleDiceRoll(sc.nextInt(), sc.nextInt()));
    ...
    private static int calculateDoubleDiceRoll(int a, int b) {
        return a % 6 + b % 6 + 2;
    }
