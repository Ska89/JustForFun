...
public class BodyMassIndex {
    private static final String[] GRADES = {"under", "normal", "over", "obese"};
    private static final double[] RANGES = {18.5, 25, 30};
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            sb.append(calculateGrades(weight, height));
            if (i < count - 1) {
                sb.append(" ");
                sc.nextLine();
            }
    ...
    private static String calculateGrades(int weight, double height) {
        double bmi = weight / Math.pow(height, 2);
        if (bmi < RANGES[0]) return GRADES[0];
        if (bmi < RANGES[1]) return GRADES[1];
        if (bmi < RANGES[2]) return GRADES[2];
        return GRADES[3];
    }
