 ...
 * Probably you've already solved the task of selecting minimum/maximum from array - this algorithm is based upon it.
 ...
 Analysis
 This algorithm needs to make about N passes through array and on each pass it performs about N/2 operations at average,
 so it has O(N^2) "time complexity", the same as for Bubble Sort. However, unlike bubble sort it does not perform about
 N/2 swaps on each pass - only a single swap is needed. This makes it work significantly faster, though for large
 datasets more cunning algorithms are required.
 ...
 You may read more in the topic on sorting algorithms comparison
...
public class SelectionSort {
    ...
    private static StringBuilder sortArray(int[] array) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < array.length - 1; i++) {
            int max = -1, maxPos = array.length - 1;
