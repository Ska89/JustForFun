 ...
 * For example, we take the first card CA and generate random value 15 for it - this means that card should be moved to
 * position 15, where D3 was - and D3 should be moved back to position 0. Then we take C2 from the position 1 and
 * generate next random number 50 - so C2 is swapped with SQ. And so on.
 * You will be given the sequence of non-negative integer random numbers - if they are greater than necessary, trim
 * them to required range by taking modulo 52, like in the Double Dice Roll task. Perform the shuffling and print the
 * new order of cards as an answer.
...
public class CardShuffing {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 52; i++)
            randoms[i] = Integer.parseInt(sc.next());
        sc.close();
        printResult(shuffleCard(fillCardsArray(), randoms));
    }

    private static String[] shuffleCard(String[] cards, int[] randoms) {
        int pos;
        String tmp;
        for (int i = 0; i < 52; i++) {
            pos = randoms[i] % 52;
    ...
    private static String[] fillCardsArray() {
        String[] cards = new String[52];
        for (int i = 0; i < RANKS.length; i++) {
            cards[i] = String.valueOf(SUITS[0]) + String.valueOf(RANKS[i]);
            cards[i + 13] = String.valueOf(SUITS[1]) + String.valueOf(RANKS[i]);
            cards[i + 26] = String.valueOf(SUITS[2]) + String.valueOf(RANKS[i]);
            cards[i + 39] = String.valueOf(SUITS[3]) + String.valueOf(RANKS[i]);
        }
        return cards;
    }
