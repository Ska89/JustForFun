 ...
 * Banking provides us with many interesting programming problems. The following one is more complicated than Savings calculator which you perhaps learned earlier.
 ...
 For example, Joel takes P = $800,000 from bank with interest rate of R = 6%. He is willing to pay M = $10000 at the
 end of each month.
 ...
 ...
 103     $4188         $21        -$4209         $0
 So after 103 months (which is about 8.5 years) the debt could be payed out. The last payment of course could be less
 than M (since P need not to become negative). Note that he will pay out about:
 ...
 Answer should contain the required monthly payment M rounded up to whole dollars (i.e. if you get non-integer result,
...
public class MortgageCalculator {
    public static void main(String[] args) throws Exception {
        ...
        printResult(calculatePayment(sc.nextInt(), sc.nextInt(), sc.nextInt()));
        sc.close();
    }

    private static long calculatePayment(int p, int r, int l) {
        double i = (double) r / 1200;
        return (long) Math.ceil(p * (i * Math.pow(1 + i, l)) / (Math.pow(1 + i, l) - 1));
    }
