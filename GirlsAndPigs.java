 ...
 *
 * Once upon a time, when I was teaching electronics in the school, I was offering the following task to newcomers in
 * order to check their manner of thinking about problems:

 The girls are pasturing pigs at the field. In total they have 106 legs and 336 breasts. How many girls and pigs are
 there?

 Pigs and girls were chosen not just for a joke! The main reason was that the number of breasts for a pig was unknown.
 So some pupils were telling that "The problem could not be solved", while others were pretending that "there are
 infinite number of solutions since we have 3 variables and 2 equations".
 ...
 Now you are to solve the "extended" version of the task. As before you will be given the number of legs and breasts
 and your goal is to tell the number of possible solutions.
...
public class GirlsAndPigs {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < count; i++) {
            sb.append(calculateSolutions(sc.nextInt(), sc.nextInt()));
    ...
    private static int calculateSolutions(int l, int b) {
        int pigs = (l - 2) / 4, count = 0;
        while (pigs > 0) {
            if (((double) (b - (l - pigs * 4)) / pigs) % 2 == 0) count++;
            pigs--;
        }
        return count;
    }
