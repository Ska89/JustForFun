 ...
 * We know that to represent letters (and any symbols) of natural language computers use some encoding which assigns character values to integers, so that any array of numbers (bytes for example) could be printed out as the sequence of characters.
 ...
 It is important that none of these codes is the prefix of any other - such coding scheme is called Prefix code and
 makes us sure that any sequence of bits could be decoded back in a unique way.

 You may later learn how such tables are constructed from Shannon-Fano Coding or Huffman Coding exercises.

 We do not use special codes for capital letters, instead let us agree that if capital letter is marked by prefixing
 it with an exclamation mark "!" - in normal text it always is followed by other punctuation symbol or space so it is
 unambigous rule.
...
public class VariableLengthCode {
    ...
    static {
        ...
        CODE_TABLE.put(' ', "11");
        CODE_TABLE.put('t', "1001");
        CODE_TABLE.put('n', "10000");
        CODE_TABLE.put('s', "0101");

        CODE_TABLE.put('r', "01000");
        CODE_TABLE.put('d', "00101");
        CODE_TABLE.put('!', "001000");
        CODE_TABLE.put('c', "000101");

        CODE_TABLE.put('m', "000011");
        CODE_TABLE.put('g', "0000100");
        CODE_TABLE.put('b', "0000010");
        CODE_TABLE.put('v', "00000001");

        CODE_TABLE.put('k', "0000000001");
        CODE_TABLE.put('q', "000000000001");
        CODE_TABLE.put('e', "101");
        CODE_TABLE.put('o', "10001");

        CODE_TABLE.put('a', "011");
        CODE_TABLE.put('i', "01001");
        CODE_TABLE.put('h', "0011");
        CODE_TABLE.put('l', "001001");

        CODE_TABLE.put('u', "00011");
        CODE_TABLE.put('f', "000100");
        CODE_TABLE.put('p', "0000101");
        CODE_TABLE.put('w', "0000011");

        CODE_TABLE.put('y', "0000001");
        CODE_TABLE.put('j', "000000001");
        CODE_TABLE.put('x', "00000000001");
        CODE_TABLE.put('z', "000000000000");
    }

    public static void main(String[] args) throws Exception {
        Scanner sc = new Scanner(System.in);
        printResult(encodeData(sc.nextLine()));
        sc.close();
    }

    private static String encodeData(String s) {
        char[] symbols = s.toCharArray();
        StringBuilder sb = new StringBuilder();
        for (char c : symbols) sb.append(CODE_TABLE.get(c));
        while (sb.length() % 8 != 0) sb.append("0");
        StringBuilder res = new StringBuilder();
        while (sb.length() > 0) {
            String hex = Integer.toHexString(Integer.parseInt(sb.substring(0, 8), 2)).toUpperCase();
            res.append(hex.length() == 2 ? hex : "0" + hex);
            res.append(" ");
            sb.delete(0, 8);
        }
