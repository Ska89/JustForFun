...
/**
 * Created by @Serhii Sydorenko on 22.10.2016.
 *
 ...
 * qualities, of which probably few most important are:
 ...
 We should search through initial sub-array and find a place where new element should be inserted. To insert it we, of
 course, need to shift all elements which are larger. See an example:

 1 3 5 6 8   - initial array
 4           - another element to add
 Searching through array we see that 4 should be inserted where 5 currently is. Let us shift all elements
 starting from 5:
 ...
 shift the part of sub-array, starting from this place - the sub-array is extended and the value which we are inserting
 is overwritten, but we have saved it inside temporary variable;
 ...
 Variants
 Comparing to Selection Sort we can see that more moves (copying) of array elements is performed, but probably less
 comparisons. So benefit of the Insertion Sort is for objects which are handled by references (as in most contemporary
 languages) but for which comparison takes significant time.

 Since we are always searching through already sorted sub-array, we can use a kind of Binary Search here, which will
 allow to decrease the amount of comparisons dramatically.
...
public class InsertionSort {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++)
            array[i] = sc.nextInt();
        sc.close();
        printResult(insertionSort(array));
    }
