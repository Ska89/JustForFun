...
/**
 * Created by @Serhii Sydorenko on 08.03.2017.
 *
 ...
 * following dedicated exercise to learn this trick.
 ...
 There are several pairs of numbers. For each pair you are to divide first by second and return the result, rounded
 to the nearest integer.

 In cases, when result contains exactly 0.5 as a fraction part, value should be rounded up (i.e. by adding another 0.5).
 Note that for negative values "greater" means "closer to zero". Refer to the Wikipedia page on Rounding for more
 thorough explanations.

 In any further problems, when rounding is mentioned - just the same rounding algorithm is supposed (unless other is
...
public class Rounding {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            sb.append((int) Math.round((double) sc.nextInt() / sc.nextInt()));
